...
class Mage_Payment_Model_Observer
{
    ...
    public function beforeOrderPaymentSave(Varien_Event_Observer $observer)
    {
        ...
        //If use multi store or multi website, when create Invoice from backend,
        //Payment Instruction overwritten by default website.
        //This code use always the orderd website's Bank Transfer Instruction.
        //kaizou for bug
        $parentId = $observer->getEvent()->getPayment()->getData('parent_id');
        $storeId;
        if($parentId){
            $storeId = Mage::getModel("sales/order")->load($parentId)->getData('store_id');
        }
        if ($payment->getMethod() === Mage_Payment_Model_Method_Banktransfer::PAYMENT_METHOD_BANKTRANSFER_CODE) {
            //kaizou for bug, always use orderd website's Bank Transfer Instructions
            $description = Mage::getStoreConfig('payment/banktransfer/instructions', $storeId);
            $payment->setAdditionalInformation('instructions',$description);
